...
namespace Nekoxy
{
    ...
    internal class TransparentProxyLogic : ProxyLogic
    {
        ...
        /// 上流プロキシ設定。
        /// </summary>
        private static ProxyConfig _UpstreamProxyConfig = new ProxyConfig(ProxyConfigType.SystemProxy);
        public static ProxyConfig UpstreamProxyConfig { 
            get {return _UpstreamProxyConfig;} 
            set {_UpstreamProxyConfig = value;}
        }
        ...
        private Uri GetEffectiveRequestUri()
        {
            ...
            var authority = isDefaultPort ? destinationHost : string.Format("{0}:{1}", destinationHost, destinationPort);
            var pathAndQuery = this.RequestLine.URI.Contains("/") ? this.RequestLine.URI : string.Empty;

            Uri uri;
            return Uri.TryCreate(string.Format("{0}://{1}/{2}", scheme, authority, pathAndQuery), UriKind.Absolute, out uri) ? uri : null;
        }
