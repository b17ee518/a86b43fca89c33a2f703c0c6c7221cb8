...
bool KanDataConnector::req_map_anchorage_repair_parse()
{
    auto jarray = _jdoc.object()["api_data"].toObject()["api_ship_data"].toArray();

    for (int i = 0; i < jarray.count(); i++)
    {
        kcsapi_ship2 api_ship;
        api_ship.ReadFromJObj(jarray[i].toObject());

        QList<kcsapi_ship2>::iterator it;
        for (it = pksd->portdata.api_ship.begin(); it != pksd->portdata.api_ship.end(); ++it)
        {
            if (api_ship.api_id == it->api_id)
            {
                *it = api_ship;
            }
        }
